<?php
Class songkok_model extends CI_Model
{
	function __constuct()
	{
	...
	function cekstok_produk($kodebarang) {
		$q = $this->db->query("SELECT stok from tbl_produk WHERE kode_produk = '".$kodebarang."'");
		return $q;
	}
	...
	{
		$q = $this->db->query("SELECT * from tbl_pengiriman ORDER BY pengiriman ASC");
		return $q;		
	}
	function side_kategori()
	{
		$q = $this->db->query("SELECT * from tbl_kategori ORDER BY nama_kategori ASC");
		return $q;		
	}
	...
	function kirim_hubungi_kami($upd)
	{
		$query=$this->db->query($upd);
	}
	function insert_barang_rusak($upd)
	{
		$query=$this->db->query($upd);
	}
	...
	{
		$query=$this->db->query("update tbl_produk set stok=stok-".$bl.", dibeli=dibeli+".$bl." where kode_produk='".$kd."'");
	}
	...
	{
		$query=$this->db->query("SELECT substring(a.kode_transaksi,1,8) as tgl, count(kode_produk) as jm, a.kode_transaksi, a.tgltransaksi FROM `tbl_transaksi_header` as a left join tbl_transaksi_detail as b on a.kode_transaksi=b.kode_transaksi where kode_user='$kd' group by tgl LIMIT $offset,$limit");
		return $query;
	}
	function tampil_det_history($kd_usr,$kd,$limit,$offset)
	{
		$query=$this->db->query("
			...
			tbl_produk.harga,
			tbl_transaksi_header.kode_transaksi,
			tbl_transaksi_header.noresi,
			tbl_transaksi_header.tgltransaksi,
			tbl_transaksi_header.hargaperkilo,
			tbl_transaksi_header.subberat,
			tbl_transaksi_header.subtotal,
			tbl_pengiriman.pengiriman, 
			tbl_kota.kota,
			tbl_transaksi_header.status
			FROM
			tbl_transaksi_detail
			INNER JOIN tbl_transaksi_header ON tbl_transaksi_header.kode_transaksi = tbl_transaksi_detail.kode_transaksi
			...
			WHERE tbl_transaksi_header.kode_user = ".$kd_usr."
			ORDER BY tbl_transaksi_header.kode_transaksi DESC 
			LIMIT ".$offset.", ".$limit."
		");
		/*$query=$this->db->query("
			SELECT
